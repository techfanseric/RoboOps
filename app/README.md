...
当前版本：`v0.2.2`。本版本新增退款 / 退单管理，覆盖父子订单关联、批量与部分退款、失败免审、策略配置、审核、退单依赖、失败重试和未知结果查询。全部使用前端仿真数据，预留小程序支付对接字段，不包含真实支付调用。验收场景与关键截图见仓库根目录 README。
...
- 商品/服务目录
- 订单/服务请求中心
- 状态与异常中心（实时状态、异常事件、状态字典、异常规则、发布记录）
- 任务/工单
- 配置发布
- 报表
- 角色权限
