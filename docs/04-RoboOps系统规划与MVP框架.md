...
本期 MVP 的目标不是一次性做完茶饮、咖啡、零售或某个具体场景，而是先搭出一套可以承载多种机器人商业场景的运营系统骨架。

核心判断：

- RoboOps 不应被定义为“茶饮后台”或“咖啡亭后台”。
- 饮品亭可以作为第一套场景模板，但不能成为系统底层硬编码。
- 品牌、点位、商品/服务、机器人/设备、履约流程、异常规则、权限角色都应尽量配置化。
- MVP 先做“可配置框架 + 一个可跑通的样板场景”，再逐步补齐垂直行业深度。
- 本期不追求完整流程引擎，而是先做轻量状态机、规则配置和模板化对象。
...
- 重点不是复刻页面，而是提炼“什么角色在什么场景处理什么状态和异常”。
...
以下问题不应理解为“等甲方来定义产品”，而是用于验证事实、选择配置和控制实施范围。
...
1. `06-PRD-v0.1.md`：把系统规划和角色权限模型转成产品需求文档。
2. `07-信息架构与页面清单.md`：定义后台菜单、页面和关键字段。
3. `08-核心数据模型ERD.md`：把实体关系画清楚，方便后端开始建模。
4. `09-MVP原型与演示数据方案.md`：定义第一版 Demo 数据、样板场景和演示流程。
